...
for mod_name, attr_name in modules_to_check:
    try:
        ...
        print(f"[CHECK] {mod_name}: FAILED — {type(e).__name__}: {e}", flush=True)
        traceback.print_exc()
        all_ok = False
...
try:
    ...
    print(f"[CHECK] engine.policy_engine: FAILED — {type(e).__name__}: {e}", flush=True)
    traceback.print_exc()
    all_ok = False
...
try:
    ...
    print(f"[STARTUP] main.app import: FAILED — {type(e).__name__}: {e}", flush=True)
    traceback.print_exc()
    all_ok = False

if not all_ok:
    print("\n[STARTUP] ERRORS DETECTED — starting minimal server anyway", flush=True)
    from fastapi import FastAPI
    app = FastAPI(title="AgentServices (degraded)")

    @app.get("/health")
    async def health():
        return {"status": "DEGRADED", "message": "Some modules failed to import"}
